<?php
class swiperSlider extends WidgetHandler
{
    function proc($args)
    {
        if (!$args->identifying_way)
            $args->identifying_way = 'generate_random_num';

        // 정렬 대상
        if (!in_array($args->order_target, array('list_order', 'update_order', 'regdate', 'rand()')))
        ...
        // 제목 폰트 크기
        if (!$args->subject_font_size)
            $args->subject_font_size = 60;

        // 내용 폰트 크기
        if (!$args->content_font_size)
            $args->content_font_size = 20;
        ...
        if (!$args->thumbnail_width)
            $args->thumbnail_width = 1920;

        // 썸네일 세로 크기
        if (!$args->thumbnail_height)
            $args->thumbnail_height = 500;

        // 보기 옵션
        $args->option_view_arr = explode(',', $args->option_view);

        // 컨텐츠 표시 위치
        if (!$args->content_position)
            $args->content_position = 5;

        // Slider Height
        if (!$args->slide_height)
            $args->slide_height = 500;
        ...
        if (!$args->hyperlink)
            $args->hyperlink = 'Y';
        if (!$args->hyperlink_src)
            $args->hyperlink_src = 'article';
        if (!$args->hyperlink_target)
            $args->hyperlink_target = 'N';
        ...
        if ($args->category_srl) {
            ...
            $args->specific_category = false;

        // 표시 권한
        $args->view_permission_arr = explode(',', $args->view_permission);
        $vp_view = false;
        $vp_list = false;
        $vp_write_document = false;
        $vp_write_comment = false;
        if (count($args->view_permission_arr)) {
            foreach ($args->view_permission_arr as $kk => $val) {
                if ($val == 'list')
                    ...
                    $vp_view = true;
                else if ($val == 'write_document')
                    $vp_write_document = true;
                else if ($val == 'write_comment')
                    $vp_write_comment = true;
            }
        }
        $vp_exist = false;
        if ($vp_view || $vp_list || $vp_write_document || $vp_write_comment)
            $vp_exist = true;

        // 비밀글 표시 방법
        if (!$args->view_secret_document)
            $args->view_secret_document = 'all_user';
        ...
        if (!$args->module_srls) {
            ...
            if ($output->data) {
                foreach ($output->data as $key => $val) {
                    ...
                    if (
                        $vp_exist && !(($vp_view && $gg->view) || ($vp_list && $gg->list) ||
                            ($vp_write_document && $gg->write_document) ||
                            ($vp_write_comment && $gg->write_comment))
                    )
                        continue;
        ...
        if (!count($args->modules_info) || !count($args->mid_lists))
    ...
    function _getDocumentItems($args)
    {
        ...
        $obj->list_count = $args->list_count == 0 ? 5 : $args->list_count; // 한번에 가져올 글 목록 개수
        if ($args->category_srl)
        ...
        while (true) {
            ...
            $obj->page = $pagecount;
            $output = executeQueryArray('widgets.swiperSlider.getNewestDocuments', $obj);
            // 더 이상 결과가 없으면 break
            ...
            foreach ($output->data as $key => $attribute) {
                ...
                if ($args->view_secret_document == 'use_permission' && $oDocument->isSecret() && !$oDocument->isGranted())
                    continue;
                else if ($args->view_secret_document == 'not_show' && $oDocument->isSecret())
                    continue;
        ...
        if ($getcount > 0) {
            ...
            for ($i = 0, $c = count($document_srls); $i < $c; $i++) {
                ...
                $content_item = new swiperSliderItem($args->module_srls_info[$module_srl]->browser_title);
                $content_item->adds($oDocument->getObjectVars());

                $content_item->setTitle($oDocument->getTitle($args->subject_cut_size));
                ...
                else if ($args->hyperlink_src == 'article')
                    $content_item->setLink(getSiteUrl($domain, '', 'document_srl', $document_srl));
                else if ($args->hyperlink_src == 'extravar') {
                    if ($oDocument->getExtraEidValue($args->hyperlink_extravar))
    ...
    function _compile($args, $content_items)
    {
        ...
        $widget_info->subject_font_size = $args->subject_font_size;
        ...
        $widget_info->slide_max_width = $args->slide_max_width;
        $widget_info->slide_height = $args->slide_height;

        $widget_info->content_position = $args->content_position;
        $widget_info->content_align = $args->content_align;
...
class swiperSliderItem extends Object
{
    ...
    function swiperSliderItem($browser_title = '')
    {
